	...
	* Set as global vars
	...
	use r_e_z_gdp_releases, clear
		...
		* Create a new frame named `r_e_encompassing` to store regression results
		frame create r_e_encompassing str32 variable int n double coef str32 test_result double pvalue

		* Loop through variables gdp_release_`i' where `i` ranges from 1 to 11
		forval i = 1/10 {

			capture confirm variable e_`i'_gdp // Check if the variable exists
				...
				capture {
					...
					* Run regression with Newey-West standard errors
					...
					if _rc == 2001 { // If regression fails due to insufficient observations
						di in red "Insufficient observations for r_`=`i'+1'_gdp"
						continue
					}

					* Extract regression results
					matrix M = r(table)
					local coef = M["b", "r_`=`i'+1'_gdp"]
					...
					* Perform the hypothesis test: H0: constant = 0 & coef = 1
					...
					if _rc == 0 { // If the test command succeeds
						...
						* Post results to the results frame
						frame post r_e_encompassing ("e_`i'_gdp") (`n') (`coef') ("`test_result'") (`pvalue')
					}
					else {
						di in red "Test failed for e_`i'_gdp"
					}
				}
			}
			else {
		...
		* Switch to the `r_e_encompassing` frame to view the stored results
		frame change r_e_encompassing

		* List the results without observation numbers and in a clean format
		list variable n coef test_result pvalue, noobs clean
