	...
	* Set as global vars
	...
	use gdp_bench_r_dummies_cleaned, clear
		...
		** Set common information using regression for model III (H1) to keep if !missing(residuals)
		...
		qui {
			tsset vintages_date
			newey r_12_gdp c.r_11_gdp##i.r_11_gdp_dummy, lag(1) force noconstant
		...
		* Create a new frame named `efficiency_bench` to store regression results
		...
		frame create efficiency_bench str32 variable int n str32 coef_1 str32 coef_2 str32 coef_3 str32
		...
		forval i = 2/12 {
			...
			if !_rc {
				...
				capture {
					...
					newey r_`i'_gdp c.r_`=`i'-1'_gdp##i.r_`=`i'-1'_gdp_dummy, lag(1) force noconstant
					...
					*** Append standard error in parentheses to coef
					...
					frame post efficiency_bench ("r_`i'_gdp") (`n') ("`coef_1'") ("`coef_2'") ("`coef_3'")
				}
			}
	...
	list variable n coef_1 coef_2 coef_3, noobs clean
		...
		* Export to excel file
		...
		export excel using "$tables_folder/gdp_efficienccy_noconstant_bench.xlsx", ///
