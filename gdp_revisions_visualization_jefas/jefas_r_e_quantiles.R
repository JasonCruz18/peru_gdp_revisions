...
# Filter data by horizon values (>1 & <13) for relevant analysis
merged_df <- merged_df %>% filter(horizon > 1 & horizon < 13)

# Filter for vintages_dates > 30/11/1992 and vintages_dates < 01/11/2023
merged_df <- merged_df %>% 
  filter(vintages_date > as.Date("1992-11-30") & vintages_date < as.Date("2023-11-01"))
...
r_gdp_plot <- ggplot(summary_data, aes(x = horizon)) +
...
output_file <- file.path(output_dir, "gdp_r_quantiles.png")

# Guardar el gráfico en alta resolución
ggsave(filename = output_file, plot = r_gdp_plot, width = 10, height = 6, dpi = 300, bg = "transparent")
