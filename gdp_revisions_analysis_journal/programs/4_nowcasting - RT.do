...
*	global output_graphs "output/graphs"	// Use to export charts.
	...
	gen byte train = (tm <= tm(2013m12)) // eval > 2013m12
	...
	* 	local delta = 0.3
	...
	use "$input_data/fitted_vals", clear
		...
		* Create an empty matrix to hold RMSE results
		...
		forvalues h = 1/11 {
			gen sq_now = (e_now_`h')^2 if train==0
		...
		* Create an empty matrix to hold MAE-based DM t-stat results
		...
		forvalues h = 1/11 {
			...
			newey d_`h'_dm_mae if train==0, lag(6) force
		...
		* Create an empty matrix to hold Encompassing t-stat results
		...
		forvalues h = 1/11 {
			...
			newey e_`h' d_`h'_encom_1 if train==0, lag(6) force
		...
		* Create an empty matrix to hold Encompassing t-stat results
		...
		forvalues h = 1/11 {
			...
			newey e_now_`h' d_`h'_encom_2 if train==0, lag(6) force
	...
	* Export directly from matrix to Excel (.xls)
	putexcel set "$output_tables/rt_nwc.xls", sheet("rt_nwc") replace
