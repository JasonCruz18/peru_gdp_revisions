...
***
	...
	* Set as global vars
	...
	odbc load, exec("select * from sectorial_gdp_annual_cum_revisions_panel_bench") dsn("gdp_revisions_datasets") lowercase sqlshow clear // Change frequency to monthly, quarterly or annual to load dataset from SQL. 
		...
		* Loop to run regressions for each sector
		...
		foreach sector of global sectors {
			...
			* Report results using esttab
			...
			esttab fe_`sector' xtscc_fe_`sector' re_`sector' xtscc_re_`sector' using "a_abs_error_bench.tex", append ///
		...
		* Loop to run regressions for each sector
		...
		foreach sector of global sectors {
			...
			* Report results using esttab
			...
			esttab fe_`sector' xtscc_fe_`sector' re_`sector' xtscc_re_`sector' using "a_abs_error_`sector'_bench.tex", ///
			b(%9.3f) se(%9.3f) stats(n_`sector' h_`sector' N_`sector', label("n" "$\bar{h}$" "N") fmt(%9.0f %9.0f %9.0f)) ///
			order(_cons) ///
			varlabels(_cons "Intercepto" horizon "h") ///
			noobs ///
		...
		* Loop to run regressions for each sector
		...
		foreach sector of global sectors {
			...
			* Report results using esttab
			...
			esttab fe_`sector' xtscc_fe_`sector' re_`sector' xtscc_re_`sector' using "a_sq_error_bench.tex", append ///
				b(%9.3f) se(%9.3f) stats(n_`sector' h_`sector' N_`sector', label("n" "$\bar{h}$" "N") fmt(%9.0f %9.0f %9.0f)) ///
				order(_cons) longtable ///
				varlabels(_cons "Intercepto" horizon "h") ///
				noobs ///
		...
		* Loop to run regressions for each sector
		...
		foreach sector of global sectors {
			...
			* Report results using esttab
			...
			esttab fe_`sector' xtscc_fe_`sector' re_`sector' xtscc_re_`sector' using "a_sq_error_`sector'_bench.tex", ///
			b(%9.3f) se(%9.3f) stats(n_`sector' h_`sector' N_`sector', label("n" "$\bar{h}$" "N") fmt(%9.0f %9.0f %9.0f)) ///
			order(_cons) ///
			varlabels(_cons "Intercepto" horizon "h") ///
			noobs ///
