	...
	use int_temp_panel_data_cleaned, clear
		...
		* global
		...
		program define dummy_process_sector_r
			args sector model_type
			local dep_var r_`sector'
			local indep_vars L1.r_`sector' L2.r_`sector' c.L1.r_`sector'#c.L1.dummy_`sector' c.L2.r_`sector'#c.L2.dummy_`sector'
		...
		foreach sector of global sectors {
			...
			esttab fe_`sector' xtscc_fe_`sector' re_`sector' xtscc_re_`sector' using "benchmark_revisions_efficiency_test.txt", append ///
				b(%9.3f) se(%9.3f) stats(chi_`sector' p_`sector' n_`sector' h_`sector' N_`sector', label("Chi2" "p" "n" "h" "N") fmt(%9.3f %9.3f %9.0f %9.0f %9.0f)) ///
				order(_cons) longtable ///
				varlabels(_cons "Intercepto" L.r_`sector' "r(-1)" L2.r_`sector' "r(-2)" cL.r_`sector'#cL.dummy_`sector' "r(-1)xDummy" cL2.r_`sector'#cL2.dummy_`sector' "r(-2)xDummy") ///
				noobs ///
				star(* 0.1 ** 0.05 *** 0.01)
		}
