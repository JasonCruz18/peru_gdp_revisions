...
***
	...
	* Set as global vars
	...
	/*----------------------
	Regression (revisions-cross-sectors)
	-----------------------*/

	* r^{PBI}_t(h) vs r^{Agriculture}_t(h-1) r^{Agriculture}_t(h-2)	
	*.........................................................................
	...
	preserve // Save the current status
		...
		foreach sector of global sectors {
			...
			esttab fe_`sector' xtscc_fe_`sector' re_`sector' xtscc_re_`sector' using "cross-sectors.txt", append ///
				b(%9.3f) se(%9.3f) stats(chi_`sector' p_`sector' n_`sector' h_`sector' N_`sector', label("Chi2" "p" "n" "h" "N") fmt(%9.3f %9.3f %9.0f %9.0f %9.0f)) ///
				order(_cons) ///
				varlabels(_cons "Intercepto" L.r_`sector' "r(-1) [`sector']" L2.r_`sector' "r(-2) [`sector']") ///
				noobs ///
				star(* 0.1 ** 0.05 *** 0.01)
		}
